(() => {
      ...
      if (preserveWS === "full" || top.inlineContext(dom) || /[^ \t\r\n\u000c]/.test(value)) {
        if (!preserveWS) {
          ...
          for (let i = 0; i < lines.length; i++) {
            ...
            if (lines[i])
              this.insertNode(schema2.text(lines[i]), marks, !/\S/.test(lines[i]));
          }
          value = "";
        } else {
  ...
  function canSplit(doc3, pos, depth = 1, typesAfter) {
    ...
    let index2 = $pos.indexAfter(base3);
    let baseType = typesAfter && typesAfter[0];
    return $pos.node(base3).canReplaceWith(index2, index2, baseType ? baseType.type : $pos.node(base3 + 1).type);
  }
  ...
  function posAtCoords(view, coords) {
    ...
    if (node) {
      ...
      let prev;
      if (webkit2 && offset && node.nodeType == 1 && (prev = node.childNodes[offset - 1]).nodeType == 1 && prev.contentEditable == "false" && prev.getBoundingClientRect().top >= coords.top)
        offset--;
  ...
  function coordsAtPos(view, pos, side) {
    ...
    let supportEmptyRange = webkit2 || gecko;
    if (node.nodeType == 3) {
  ...
  var unsetMark = (typeOrName, options = {}) => ({ tr, state, dispatch }) => {
    ...
    if (empty2 && extendEmptyMarkRange) {
      ...
      const attrs2 = (_a2 = $from.marks().find((mark) => mark.type === type)) === null || _a2 === void 0 ? void 0 : _a2.attrs;
        ...
        new Plugin({
          ...
          props: {
            handlePaste: (view, event) => {
              ...
              const { tr, schema: schema2 } = view.state;
              const textNode = schema2.text(text2.replace(/\r\n?/g, "\n"));
  ...
  var TaskItem = Node3.create({
    ...
    addNodeView() {
      return ({ node, HTMLAttributes, getPos, editor }) => {
        ...
        const updateA11Y = () => {
          var _a2, _b;
          checkbox.ariaLabel = ((_b = (_a2 = this.options.a11y) === null || _a2 === void 0 ? void 0 : _a2.checkboxLabel) === null || _b === void 0 ? void 0 : _b.call(_a2, node, checkbox.checked)) || `Task item checkbox for ${node.textContent || "empty task item"}`;
        };
        updateA11Y();
...
`;
  ...
  var MarkdownSerializerState = class {
    ...
    */
    renderInline(parent, fromBlockStart = true) {
      ...
      let progress = (node, offset, index2) => {
        ...
        if (node && node.isText && marks.some((mark) => {
          let info = this.getMark(mark.type.name);
          return info && info.expelEnclosingWhitespace && !this.isMarkAhead(parent, index2 + 1, mark);
        })) {
    ...
    */
    ...
    @internal
    */
    isMarkAhead(parent, index2, mark) {
      for (; ; index2++) {
        if (index2 >= parent.childCount)
          return false;
        let next = parent.child(index2);
  ...
  function trimInline(text2, delim, from2, to) {
    let state = {
      text: text2,
      from: from2,
      to
...
`;
  ...
  var Blockquote$1 = Blockquote2.extend({
    /**
  ...
  var CodeBlock$1 = CodeBlock2.extend({
    /**
     * @return {{markdown: MarkdownNodeSpec}}
     */
    addStorage() {
  ...
  var HardBreak$1 = HardBreak2.extend({
    /**
     * @return {{markdown: MarkdownNodeSpec}}
     */
    addStorage() {
  ...
  var HorizontalRule$1 = HorizontalRule2.extend({
    /**
     * @return {{markdown: MarkdownNodeSpec}}
     */
    addStorage() {
  ...
  var Paragraph$1 = Paragraph2.extend({
    /**
     * @return {{markdown: MarkdownNodeSpec}}
     */
    addStorage() {
  ...
  var slashQuery = "";
  var slashRange = null;
  var filteredCommands = slashCommands;
  ...
  function renderSlashMenu() {
    ...
    filteredCommands = q ? slashCommands.filter(
    ...
    slashMenuEl.innerHTML = filteredCommands.map(
      (cmd, i) => `<div class="slash-item${i === slashSelectedIndex ? " selected" : ""}" data-index="${i}">
          ...
          props: {
            ...
            handleTextInput(view, from2, to, text2) {
              if (text2 === "/") {
                ...
                if (textBefore.trim() === "") {
                  setTimeout(() => showSlashMenu(from2 + 1), 0);
                }
          ...
          props: {
            handleTextInput(view, from2, to, text2) {
              ...
              if ($from.parent.type.name === "paragraph" && (textBefore === "[]" || textBefore === "[ ]")) {
                ...
                const blockStart = from2 - textBefore.length;
                const tr = state.tr.delete(blockStart, from2 + 1);
                view.dispatch(tr);
                editor.chain().focus().toggleTaskList().run();
                return true;
              }
    ...
    switch (node.type) {
      ...
      case "bulletList":
        return (node.content || []).map(
          (child) => serializeNode(child, { ...context, listType: "bullet" })
        ).join("\n");
      case "orderedList": {
      ...
      case "taskList":
        return (node.content || []).map(
          (child) => serializeNode(child, { ...context, listType: "task" })
        ).join("\n");
      case "taskItem": {
      ...
      case "codeBlock": {
        ...
        const code2 = (node.content || []).map((c) => c.text || "").join("");
        return "```" + lang + "\n" + code2 + "\n```";
      }
      case "horizontalRule":
        return "---";
  ...
  function serializeChildren(content, context) {
    ...
    for (let i = 0; i < content.length; i++) {
      ...
      const text2 = serializeNode(node, context);
      blocks.push(text2);
    }
    const lines = [];
    for (let i = 0; i < blocks.length; i++) {
      ...
      if (i < blocks.length - 1) {
        ...
        const isConsecutiveList = curr === "bulletList" && next === "bulletList" || curr === "orderedList" && next === "orderedList" || curr === "taskList" && next === "taskList";
  ...
  function initEditor() {
    ...
    const editor = new Editor({
      ...
      onUpdate({ editor: editor2 }) {
        const markdown = editorToMarkdown(editor2);
        try {
